# plot.PCA() without the legend option
mod.plot.PCA <- 
  function (x, axes = c(1, 2), choix = c("ind", "var"), ellipse = NULL, 
            xlim = NULL, ylim = NULL, habillage = "none", col.hab = NULL, 
            col.ind = "black", col.ind.sup = "blue", col.quali = "magenta", 
            col.quanti.sup = "blue", col.var = "black", label = c("all", 
                                                                  "none", "ind", "ind.sup", "quali", "var", "quanti.sup"), 
            invisible = c("none", "ind", "ind.sup", "quali", "var", "quanti.sup"), 
            lim.cos2.var = 0, title = NULL, palette = NULL, autoLab = c("auto", 
                                                                        "yes", "no"), new.plot = FALSE, select = NULL, unselect = 0.7, 
            shadowtext = FALSE, ...) 
  {
    res.pca <- x
    ...
    if (autoLab == "no") 
      autoLab = FALSE
    label <- match.arg(label, c("all", "none", "ind", "ind.sup", 
                                "quali", "var", "quanti.sup"), several.ok = TRUE)
    ...
    if ("ind.sup" %in% label) 
      lab.ind.sup <- TRUE
    lab.x <- paste("Dim ", axes[1], " (", format(res.pca$eig[axes[1], 
                                                             2], nsmall = 2, digits = 2), "%)", sep = "")
    lab.y <- paste("Dim ", axes[2], " (", format(res.pca$eig[axes[2], 
                                                             2], nsmall = 2, digits = 2), "%)", sep = "")
    if (choix == "ind") {
      if (is.null(title)) 
      ...
      if (!is.null(res.pca$ind.sup)) 
        coord.illu <- res.pca$ind.sup$coord[, axes, drop = FALSE]
      if (!is.null(res.pca$quali.sup)) 
        coord.quali <- res.pca$quali.sup$coord[, axes, drop = FALSE]
      if (!is.null(ellipse)) 
      ...
      if (is.null(xlim)) {
        ...
        if (is.na(test.invisible[1])) 
          xmin <- min(xmin, coord.actif[, 1])
        if (is.na(test.invisible[1])) 
          xmax <- max(xmax, coord.actif[, 1])
        if (!is.null(coord.illu) & is.na(test.invisible[2])) 
          xmin <- min(xmin, coord.illu[, 1])
        if (!is.null(coord.illu) & is.na(test.invisible[2])) 
          xmax <- max(xmax, coord.illu[, 1])
        if (!is.null(coord.quali) & is.na(test.invisible[3])) 
          xmin <- min(xmin, coord.quali[, 1])
        if (!is.null(coord.quali) & is.na(test.invisible[3])) 
          xmax <- max(xmax, coord.quali[, 1])
        if (!is.null(coord.ellipse) & is.na(test.invisible[1])) 
          xmin <- min(xmin, coord.ellipse[, 2])
        if (!is.null(coord.ellipse) & is.na(test.invisible[1])) 
          xmax <- max(xmax, coord.ellipse[, 2])
      ...
      if (is.null(ylim)) {
        ymin <- ymax <- 0
        if (is.na(test.invisible[1])) 
          ymin <- min(ymin, coord.actif[, 2])
        if (is.na(test.invisible[1])) 
          ymax <- max(ymax, coord.actif[, 2])
        if (!is.null(coord.illu) & is.na(test.invisible[2])) 
          ymin <- min(ymin, coord.illu[, 2])
        if (!is.null(coord.illu) & is.na(test.invisible[2])) 
          ymax <- max(ymax, coord.illu[, 2])
        if (!is.null(coord.quali) & is.na(test.invisible[3])) 
          ymin <- min(ymin, coord.quali[, 2])
        if (!is.null(coord.quali) & is.na(test.invisible[3])) 
          ymax <- max(ymax, coord.quali[, 2])
        if (!is.null(coord.ellipse) & is.na(test.invisible[1])) 
          ymin <- min(ymin, coord.ellipse[, 3])
        if (!is.null(coord.ellipse) & is.na(test.invisible[1])) 
          ymax <- max(ymax, coord.ellipse[, 3])
      ...
      if (!is.null(select)) {
        if (mode(select) == "numeric") 
          ...
          if (sum(rownames(res.pca$ind$coord) %in% select) + 
                sum(rownames(res.pca$ind.sup$coord) %in% select) != 
                0) 
            selection <- which(rownames(res.pca$ind$coord) %in% 
                                 select)
          else {
            if (grepl("contrib", select)) 
              selection <- (rev(order(res.pca$ind$contrib[, 
                                                          axes[1], drop = FALSE] * res.pca$eig[axes[1], 
                                                                                               1] + res.pca$ind$contrib[, axes[2], drop = FALSE] * 
                                        res.pca$eig[axes[2], 1])))[1:min(nrow(res.pca$ind$coord), 
                                                                         sum(as.integer(unlist(strsplit(select, 
                                                                                                        "contrib"))), na.rm = T))]
            if (grepl("dist", select)) 
              selection <- (rev(order(res.pca$ind$dist)))[1:min(nrow(res.pca$ind$coord), 
                                                                sum(as.integer(unlist(strsplit(select, 
                                                                                               "dist"))), na.rm = T))]
            if (grepl("coord", select)) 
              selection <- (rev(order(apply(res.pca$ind$coord[, 
                                                              axes]^2, 1, sum))))[1:min(nrow(res.pca$ind$coord), 
                                                                                        sum(as.integer(unlist(strsplit(select, 
                                                                                                                       "coord"))), na.rm = T))]
            if (grepl("cos2", select)) {
              if (sum(as.numeric(unlist(strsplit(select, 
                                                 "cos2"))), na.rm = T) >= 1) 
                selection <- (rev(order(apply(res.pca$ind$cos2[, 
                                                               axes], 1, sum))))[1:min(nrow(res.pca$ind$coord), 
                                                                                       sum(as.numeric(unlist(strsplit(select, 
                                                                                                                      "cos2"))), na.rm = T))]
              else selection <- which(apply(res.pca$ind$cos2[, 
      ...
      if ((!is.null(select)) & (!is.null(res.pca$ind.sup$coord))) {
        if (mode(select) == "numeric") 
          ...
          if (sum(rownames(res.pca$ind$coord) %in% select) + 
                sum(rownames(res.pca$ind.sup$coord) %in% select) != 
                0) 
            selectionS <- which(rownames(res.pca$ind.sup$coord) %in% 
                                  select)
          else {
            if (grepl("dist", select)) 
              selectionS <- (rev(order(res.pca$ind.sup$dist)))[1:min(nrow(res.pca$ind.sup$coord), 
                                                                     sum(as.integer(unlist(strsplit(select, 
                                                                                                    "dist"))), na.rm = T))]
            if (grepl("coord", select)) 
              selectionS <- (rev(order(apply(res.pca$ind.sup$coord[, 
                                                                   axes]^2, 1, sum))))[1:min(nrow(res.pca$ind.sup$coord), 
                                                                                             sum(as.integer(unlist(strsplit(select, 
                                                                                                                            "coord"))), na.rm = T))]
            if (grepl("cos2", select)) {
              if (sum(as.numeric(unlist(strsplit(select, 
                                                 "cos2"))), na.rm = T) >= 1) 
                selectionS <- (rev(order(apply(res.pca$ind.sup$cos2[, 
                                                                    axes, drop = FALSE], 1, sum))))[1:min(nrow(res.pca$ind.sup$coord), 
                                                                                                          sum(as.numeric(unlist(strsplit(select, 
                                                                                                                                         "cos2"))), na.rm = T))]
              else selectionS <- which(apply(res.pca$ind.sup$cos2[, 
                                                                  axes, drop = FALSE], 1, sum) > sum(as.numeric(unlist(strsplit(select, 
                                                                                                                                "cos2"))), na.rm = T))
            }
            if (is.integer(select)) 
              selectionS <- select
      ...
      if ((new.plot) & !nzchar(Sys.getenv("RSTUDIO_USER_IDENTITY"))) 
        dev.new(width = min(14, 8 * (xmax - xmin)/(ymax - 
                                                     ymin)), height = 8)
      if (is.null(palette)) 
      ...
      if (habillage == "ind") {
        ...
        if (length(col.hab) != nb.prod) 
          color.ind <- c(1:nb.prod)
        else color.ind <- col.hab
        if (!is.null(coord.illu)) 
          color.ind.sup <- c((nb.prod + 1):(nb.prod + nrow(coord.illu)))
        color.mod <- "darkred"
      }
      if ((habillage != "none") & (habillage != "ind")) {
        ...
        n.mod <- res.pca$call$quali.sup$modalite[liste.quali == 
                                                   nom.quali]
        if (length(col.hab) != n.mod) {
          color.mod <- c(1:n.mod)
          color.ind <- as.numeric(as.factor(res.pca$call$X[, 
                                                           nom.quali]))
          col.ind.sup <- color.ind[res.pca$call$ind.sup]
          if (!is.null(res.pca$call$ind.sup)) 
            color.ind <- color.ind[-res.pca$call$ind.sup]
        }
        else {
          color.mod <- col.hab
      ...
      if (habillage == "none") {
        ...
        color.mod <- col.quali
        if (!is.null(res.pca$ind.sup)) 
          col.ind.sup <- rep(col.ind.sup, nrow(res.pca$ind.sup$coord))
      }
      ...
      if (!is.null(res.pca$ind.sup) & is.na(test.invisible[2])) {
        ...
        if (lab.ind.sup) {
          labe2 <- rownames(res.pca$ind.sup$coord)
        }
      ...
      if (!is.null(ellipse)) {
        ...
        for (e in 1:nbre.ellipse) {
          data.elli <- coord.ellipse[ellipse$res[, 1] == 
                                       levels(coord.ellipse[, 1])[e], -1]
          if ((habillage != "none") & (habillage != "ind")) 
            lines(x = data.elli[, 1], y = data.elli[, 2], 
                  col = color.mod[e], ...)
          else lines(x = data.elli[, 1], y = data.elli[, 
                                                       2], col = col.quali, ...)
        }
      }
#       if ((habillage != "none") & (habillage != "ind")) 
#         legend("topleft", legend = levels(res.pca$call$X[, 
#                                                          habillage]), text.col = color.mod, cex = par("cex") * 
#                  0.8)
    }
    if (choix == "var") {
      if (is.null(title)) 
      ...
      if (!is.null(select)) {
        if (mode(select) == "numeric") 
          ...
          if (sum(rownames(res.pca$var$coord) %in% select) + 
                sum(rownames(res.pca$quanti.sup$coord) %in% 
                      select) != 0) 
            selection <- which(rownames(res.pca$var$coord) %in% 
                                 select)
          else {
            if (grepl("contrib", select)) 
              selection <- (rev(order(res.pca$var$contrib[, 
                                                          axes[1]] * res.pca$eig[axes[1], 1] + res.pca$var$contrib[, 
                                                                                                                   axes[2]] * res.pca$eig[axes[2], 1])))[1:min(nrow(res.pca$var$coord), 
                                                                                                                                                               sum(as.integer(unlist(strsplit(select, 
                                                                                                                                                                                              "contrib"))), na.rm = T))]
            if (grepl("coord", select)) 
              selection <- (rev(order(apply(res.pca$var$coord[, 
                                                              axes]^2, 1, sum))))[1:min(nrow(res.pca$var$coord), 
                                                                                        sum(as.integer(unlist(strsplit(select, 
                                                                                                                       "coord"))), na.rm = T))]
            if (grepl("cos2", select)) {
              if (sum(as.numeric(unlist(strsplit(select, 
                                                 "cos2"))), na.rm = T) >= 1) 
                selection <- (rev(order(apply(res.pca$var$cos2[, 
                                                               axes], 1, sum))))[1:min(nrow(res.pca$var$coord), 
                                                                                       sum(as.numeric(unlist(strsplit(select, 
                                                                                                                      "cos2"))), na.rm = T))]
              else selection <- which(apply(res.pca$var$cos2[, 
                                                             axes], 1, sum) > sum(as.numeric(unlist(strsplit(select, 
                                                                                                             "cos2"))), na.rm = T))
            }
            if (is.integer(select)) 
      ...
      if ((!is.null(select)) & (!is.null(res.pca$quanti.sup))) {
        if (mode(select) == "numeric") 
          ...
          if (sum(rownames(res.pca$var$coord) %in% select) + 
                sum(rownames(res.pca$quanti.sup$coord) %in% 
                      select) != 0) 
            selectionS <- which(rownames(res.pca$quanti.sup$coord) %in% 
                                  select)
          else {
            if (grepl("contrib", select)) 
              selectionS <- NULL
            if (grepl("coord", select)) 
              selectionS <- (rev(order(apply(res.pca$quanti.sup$coord[, 
                                                                      axes]^2, 1, sum))))[1:min(nrow(res.pca$quanti.sup$coord), 
                                                                                                sum(as.integer(unlist(strsplit(select, 
                                                                                                                               "coord"))), na.rm = T))]
            if (grepl("cos2", select)) {
              if (sum(as.numeric(unlist(strsplit(select, 
                                                 "cos2"))), na.rm = T) >= 1) 
                selectionS <- (rev(order(apply(res.pca$quanti.sup$cos2[, 
                                                                       axes], 1, sum))))[1:min(nrow(res.pca$var$coord), 
                                                                                               sum(as.numeric(unlist(strsplit(select, 
                                                                                                                              "cos2"))), na.rm = T))]
              else selectionS <- which(apply(res.pca$quanti.sup$cos2[, 
                                                                     axes], 1, sum) > sum(as.numeric(unlist(strsplit(select, 
                                                                                                                     "cos2"))), na.rm = T))
            }
            if (is.integer(select)) 
      ...
      if (!is.null(res.pca$quanti.sup)) 
        coord.quanti <- res.pca$quanti.sup$coord[, axes, 
                                                 drop = FALSE]
      ...
      if (is.null(palette)) 
        palette(c("black", "red", "green3", "blue", "cyan", 
                  ...
                  "maroon"))
      if (scale.unit) {
        plot(0, 0, xlab = lab.x, ylab = lab.y, xlim = xlim, 
             ylim = ylim, col = "white", asp = 1, main = titre, 
             ...)
      ...
      if (!is.null(coord.var[which(apply(res.pca$var$cos2[, 
                                                          axes, drop = FALSE], 1, sum, na.rm = TRUE) >= lim.cos2.var), 
                             ]) & is.na(test.invisible[1]) & (nrow(coord.var) > 
                                                                0)) {
        coord.var <- coord.var[which(apply(res.pca$var$cos2[, 
                                                            axes, drop = FALSE], 1, sum, na.rm = TRUE) >= 
                                       lim.cos2.var), , drop = FALSE]
        coo <- coord.var
        col.var <- rep(col.var, nrow(coord.var))
        coll <- c(coll, col.var)
        if (lab.var) {
          labe <- c(labe, rownames(coord.var))
        }
      ...
      if (!is.null(coord.quanti)) {
        if (!is.null(coord.quanti[which(apply(res.pca$quanti.sup$cos2[, 
                                                                      axes, drop = FALSE], 1, sum, na.rm = TRUE) >= 
                                          lim.cos2.var), ]) & is.na(test.invisible[2]) & 
              (nrow(coord.quanti) > 0)) {
          coord.quanti <- coord.quanti[which(apply(res.pca$quanti.sup$cos2[, 
                                                                           axes, drop = FALSE], 1, sum, na.rm = TRUE) >= 
                                               lim.cos2.var), , drop = FALSE]
      ...
      if (any(labe != "")) {
        ...
        if (autoLab == FALSE) 
          text(coo[labe != "", 1], y = coo[labe != "", 
                                           2], labels = labe[labe != ""], pos = posi[labe != 
                                                                                       ""], col = coll[labe != ""], ...)
        if (autoLab == TRUE) 
          autoLab(coo[labe != "", 1], y = coo[labe != "", 
                                              2], labels = labe[labe != ""], col = coll[labe != 
